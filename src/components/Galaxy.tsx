...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      vec3 orangeTint = vec3(1.0, 0.42, 0.08);
      vec3 color = orangeTint * val;
...
void main() {
  ...
  vec3 col = baseBg + starCol;

  if (uTransparent) {
    float alpha = length(starCol);
    alpha = smoothstep(0.0, 0.25, alpha);
    alpha = min(alpha + 0.95, 1.0);
